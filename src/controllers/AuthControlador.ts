  ...
  static async login(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!idToken) {
        ...
        res.status(400).json({ error: "Se requiere idToken de Firebase" });
        return;
      }
      ...
      if (!email) {
        ...
        res.status(400).json({ error: "El token no contiene información de email" });
        return;
      }
      ...
      if (!usuario) {
        ...
        const newUser = await UsuarioRepositorio.crearDesdeGoogle({
          correo: email,
          firebaseUid: decodedToken.uid,
          nombreCompleto: decodedToken.name || "",
